...
> 该文件为全量初始化（已合并原 0001~0012 及 Buff 集单快照 / 管理员权限链 / 工程保护等后续增量），全新数据库一次执行即可；已按旧迁移初始化过的库请勿重跑。后续增量（如快照合并 `0003_squash_snapshot.sql`、标准词条集 `0004_standard_substat_sets.sql`）为已部署库的升级补丁，全新库需在 `0001_init.sql` 之后补执行 `0004_standard_substat_sets.sql`。
...
- **标准词条集**：工具箱拉取本平台 `/api/substat-sets` 的「特殊角色」整份方案（5 部位主词条 + 副主词条 + 共 14 条副词条），用于一键把角色 5 个声骸改标准词条；其余角色的方案由工具箱按角色数据本地生成。维护入口：管理员 `/admin/substat-sets`
