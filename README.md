...
- **标准词条集** — 特殊角色的整份声骸词条方案（5 部位主词条 + 副主词条 + 共 14 条副词条，即「标准 14 词条」）；管理员在 `/admin/substat-sets` 用 5 槽位编辑器或 JSON 导入维护。工具箱在配装页「词条方案 → 工坊同步」或首次进入的同步弹窗中拉取；一般角色的方案由工具箱按角色数据本地生成
