    ...
    return (
        <>
            <button
                ...
                className="inline-flex items-center gap-1 rounded-none border border-(--card-border) bg-(--card) px-3 py-1.5 text-sm text-(--muted) transition-colors hover:text-(--fg)"
            ...
            {open && (
                <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center sm:p-4">
                    ...
                    <div
                        ...
                        tabIndex={-1}
                        className="relative flex max-h-[calc(100dvh-1rem)] w-full select-none flex-col overflow-hidden rounded-none border border-(--card-border) bg-(--card) p-4 pb-[max(1rem,env(safe-area-inset-bottom))]  outline-none sm:max-w-md rounded-none sm:p-5"
                    >
                        ...
                        <div className="mt-4">
                            ...
                            <input
                                ...
                                className="min-h-11 w-full select-text rounded-none border border-(--card-border) bg-(--input-bg) px-3 py-2 font-mono text-base outline-none transition-colors focus:border-(--accent) sm:min-h-0 sm:text-sm"
                            />
                        ...
                        <div className="mt-4 min-h-0">
                            ...
                            <output
                                className="block max-h-28 select-text overflow-y-auto rounded-none border border-(--card-border) bg-(--card-hover) px-3 py-2.5 font-mono text-[11px] leading-5 break-all text-(--muted)"
                                title={link}
                            >
                                {link}
                            </output>
                        ...
                        <div className="mt-4 grid grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)] gap-2 sm:flex sm:justify-end">
                            <button
                                onClick={() => setOpen(false)}
                                className="min-h-11 rounded-none border border-(--card-border) bg-(--card) px-3 py-2 text-sm text-(--muted) transition-colors hover:bg-(--card-hover) hover:text-(--fg) sm:min-h-0 sm:py-1.5"
                            >
                                取消
                            </button>
                            <button
                                onClick={copy}
                                className="inline-flex min-h-11 items-center justify-center gap-1.5 rounded-none px-4 py-2 text-sm font-medium border border-(--card-border) bg-(--btn-bg) text-(--btn-text) transition-colors hover:bg-(--card) hover:text-(--fg) transition-all   sm:min-h-0 sm:py-1.5"
                            >
