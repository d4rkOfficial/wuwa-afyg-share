    ...
    return (
        <>
            ...
            <button
                ...
                className="flex w-full items-center gap-2.5 rounded-none border border-(--card-border) bg-(--card) px-4 py-3 text-left transition-colors hover:border-(--accent) hover:bg-(--card-hover)"
                title="查看全部公告"
            >
                ...
                createPortal(
                    <div className="fixed inset-0 z-[70]">
                        ...
                        <div className="fixed inset-0 flex items-center justify-center p-4">
                            <div className="relative flex max-h-[80vh] w-[min(96vw,560px)] flex-col overflow-hidden rounded-none border border-(--card-border) bg-(--card) ">
                            ...
                            <div className="flex items-center gap-2">
                                {isAdmin && !creating && editingId === null && (
                                    <button
                                        ...
                                        className="inline-flex items-center gap-1 rounded-none px-2.5 py-1 text-xs font-medium border border-(--card-border) bg-(--btn-bg) text-(--btn-text) transition-colors hover:bg-(--card) hover:text-(--fg) transition-all "
                                    >
                            ...
                            {(creating || editingId !== null) && (
                                <div className="space-y-2 rounded-none border border-(--accent) bg-(--input-bg) p-3">
                                    <input
                                        ...
                                        placeholder="公告标题…"
                                        autoFocus
                                        className="w-full rounded-none border border-(--card-border) bg-(--input-bg) px-2.5 py-2 text-sm font-medium outline-none focus:border-(--accent)"
                                    />
                                    <textarea
                                        value={draftContent}
                                    ...
                                    <div className="flex items-center justify-end gap-2">
                                        ...
                                        <button
                                            ...
                                            className="rounded-none px-3 py-1 text-xs font-medium border border-(--card-border) bg-(--btn-bg) text-(--btn-text) transition-colors hover:bg-(--card) hover:text-(--fg) transition-all  disabled:opacity-50"
                                        >
                                            {busy ? '保存中…' : '保存'}
                                        </button>
                                    </div>
                                                ...
                                                <div className="flex items-center gap-1">
                                                    {editing ? (
                                                        ...
                                                        <>
                                                            ...
                                                            <button
                                                                onClick={() => remove(a.id)}
                                                                className="rounded border border-(--danger) px-1.5 py-0.5 text-[11px] font-black tracking-tight text-(--danger)"
                                                            >
                                                                确认删除
                                                            </button>
                                                        </>
                                        ...
                                        {editing ? (
                                            <>
                                                <input
                                                    ...
                                                    autoFocus
                                                    placeholder="公告标题"
                                                    className="mb-2 w-full rounded-none border border-(--card-border) bg-(--input-bg) px-2.5 py-2 text-sm font-medium outline-none focus:border-(--accent)"
                                                />
                                                <textarea
                                                    value={draftContent}
                                                    onChange={(e) => setDraftContent(e.target.value)}
