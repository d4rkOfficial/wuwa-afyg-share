// ── nanoka → 规范化契约 转换 ─────────────────────────────────────────────
// 把 nanoka 原始文件结构转换为 @/lib/upstream/types 的规范化契约。
// 仅 nanoka 适配器使用；其它上游各自实现自己的转换。
// 移植自 wuwa-afyg-tool/src/lib/api/provider/nanoka/utils.ts。

import { ASSET_BASE } from './consts'
import { ELEMENT_MAP, WEAPON_TYPE_MAP, COST_MAP } from '@/lib/upstream/game-terms'
import type {
    Character,
    Weapon,
    Echo,
