...
// 移植自 wuwa-afyg-tool/src/lib/api/provider/nanoka/consts.ts，去除对 $app/environment 的依赖。
...
let _wwVersion = isBrowser ? window.localStorage.getItem(VERSION_KEY) ?? '3.5' : '3.5'
        ...
        _versionPromise = (async () => {
            try {
                ...
                if (text) {
                    ...
                    if (isBrowser) window.localStorage.setItem(VERSION_KEY, _wwVersion)
                }
            } catch {
                // keep fallback
            }
