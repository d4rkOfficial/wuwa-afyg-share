...
1. 条件（方括号内，按 condition/实体如实标注，用「,」分隔）：
   ...
   - 某某角色[N链]：该 buff 需某角色 ≥N 链才生效时标注（与 condition.type=chain 对应）。
   - 某某武器N阶：该 buff 需某武器 ≥N 阶精炼才生效时标注（与 condition.type=refinement 对应）。
2. 触发（尖括号内）：
...
- 曙色天光可叠 2 层 → 拆两条：[散]<引爆冰棱>全队+攻击2层 + [散]<引爆冰棱>全队+攻击1层（每层填累计数值）

叠层拆分：同一增益分多层/多阶生效（如"每层+5%，可叠4层""可叠加2层"），必须拆成多条独立 buff，
buff 名用层数区分（XXXX1层、XXXX2层…），每层填该层累计数值，不要合并。
...
输出：
{"buffs":[{"buffName":"[散]<施放第5段普攻>暴击率","scope":"self","exclusive":false,"zones":[{"zoneId":"critRate","value":15,"ref":null,"override":false}]},{"buffName":"[散]<造成伤害,目标生命低于70%>增伤","scope":"self","exclusive":false,"zones":[{"zoneId":"bonusDmg","value":35,"ref":null,"override":false}]},{"buffName":"[散]<引爆冰棱>全队+攻击2层","scope":"team","exclusive":false,"zones":[{"zoneId":"atkPct","value":10,"ref":null,"override":false}]},{"buffName":"[散]<引爆冰棱>全队+攻击1层","scope":"team","exclusive":false,"zones":[{"zoneId":"atkPct","value":20,"ref":null,"override":false}]}]}
说明："自身"→scope=self；"队伍中的角色"→scope=team 且名字带"全队"；"可叠加2层"→拆成 1层/2层 两条（每层填累计数值）。

—— 示例3（武器效果，叠层拆分）——
输入：{"effect":{"desc":"攻击提升15%。造成伤害时获得灼羽，每层使共鸣技能伤害加成提升5%，可叠14层"}}
输出：
{"buffs":[{"buffName":"[赫奕]<常驻>攻击","scope":"self","exclusive":false,"zones":[{"zoneId":"atkPct","value":15,"ref":null,"override":false}]},{"buffName":"[赫奕]<造成伤害>增伤(共鸣技能)2层","scope":"self","exclusive":false,"zones":[{"zoneId":"bonusDmg","value":5,"ref":null,"override":false}]},{"buffName":"[赫奕]<造成伤害>增伤(共鸣技能)1层","scope":"self","exclusive":false,"zones":[{"zoneId":"bonusDmg","value":10,"ref":null,"override":false}]}]}
...
输出：
{"buffs":[{"buffName":"[散]<常驻>攻击","scope":"self","exclusive":false,"zones":[{"zoneId":"atkPct","value":8,"ref":null,"override":false}]},{"buffName":"[散[3链]]<常驻>暴击伤害","scope":"self","exclusive":false,"condition":{"type":"chain","min":3},"zones":[{"zoneId":"critDmg","value":20,"ref":null,"override":false}]}]}
说明：第1链视为基础配置不设 condition；"第3链"明确有门槛 → condition={"type":"chain","min":3}，名字条件带 [散[3链]]。

—— 示例8（武器精炼，含生效条件 refinement）——
输入：{"effect":{"desc":"攻击提升15%。精炼5阶时，共鸣技能伤害额外提升10%"}}
输出：
{"buffs":[{"buffName":"[赫奕]<常驻>攻击","scope":"self","exclusive":false,"zones":[{"zoneId":"atkPct","value":15,"ref":null,"override":false}]},{"buffName":"[赫奕5阶]<常驻>增伤(共鸣技能)","scope":"self","exclusive":false,"condition":{"type":"refinement","min":5},"zones":[{"zoneId":"bonusDmg","value":10,"ref":null,"override":false}]}]}
说明：武器基础效果无门槛不设 condition；"精炼5阶时"的额外效果 → condition={"type":"refinement","min":5}，名字条件带 [赫奕5阶]。`
